...
class TaxPortalCategoryModel extends Model
{
    protected $type = [
        'more' => 'array',
    ];
...
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['url']     = cmf_url('AdminTaxCategory/index', ['id' => $item['id']]);;
            $item['str_action'] = '<a href="' . url("AdminTaxCategory/add", ["parent" => $item['id']]) . '">添加子分类</a>  <a href="' . url("AdminTaxCategory/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="js-ajax-delete" href="' . url("AdminTaxCategory/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
            array_push($newCategories, $item);
        }
